...
//we no longer care about the peole that donated less than $100 so create a new array of those who donated a 100 and above.
// Problem 1
let bestDonaters = runners.filter(function(currentValue) {
  return currentValue.donation > 100;
});
console.log(bestDonaters);

//make all the runners that have a small shirt now have a large shirt
// Problem 2
let skinnyKidsBeFatKids = runners.map(function(currentValue) {
  if (currentValue.shirt_size === "S") {
    currentValue.shirt_size = "L";
  }
  return currentValue;
});
console.log(skinnyKidsBeFatKids);

//make a new list of the company ans email
// Problem 3
let bussinessInfo = [];
runners.forEach(function(items) {
  return bussinessInfo.push(` ${items.first_name} "," ${items.email} `);
});

console.log(bussinessInfo.sort());
